...
| `web::dashboard` (sessions) | `play_sessions` read path — `GET /api/sessions/live` (derived, liveness-checked) + `GET /api/sessions/history` — plus the one mutation, `POST /api/sessions/{session_key}/terminate` (MACT-02), which relays a `CastController::stop` for a resolved live session | [HTTP API surface](docs/reference/http-api-surface-25-routes.md) |
