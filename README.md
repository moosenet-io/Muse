...
2. **Still-liveness.** `matching::liveness::check_liveness` runs cheap heuristics directly on
   the sample stills' raw JPEG bytes: mean/variance of the raw byte values plus a
   dominant-byte-ratio (rejecting near-uniform/black/slate stills), and a cross-still
   comparison (rejecting a set of stills that all look identical — a stuck/frozen source).
   **Deliberately does not decode JPEG pixels** — this crate has no image-decode dependency,
   and a full decode isn't needed for this signal: a solid-color/black frame's DCT blocks are
   almost entirely "all zero AC coefficients", so the JPEG entropy coder itself produces a
   highly repetitive byte stream, which the raw-byte statistics already catch cheaply. This is
   a documented simplification (see `liveness.rs`'s module doc comment) — a real pixel decode
   remains a drop-in future swap behind the same `StillStats`/`LivenessOutcome` API if the
   heuristic ever proves too coarse (e.g. on a genuinely correct but visually atypical title
   like black-and-white content).
...
is nothing to judge (e.g. no stills at all), and still a clear `Inconsistent` on a hard
metadata contradiction — a mislabeled file doesn't get a free pass just because no vision
...
- `vlm_absent_with_no_stills_is_inconclusive_not_a_crash` — no stills at all → `Inconclusive`,
  not a panic or a false `Consistent`.
